...
public class CollisionGUI extends InteractiveGUI {
	...
	private void findColliders() {
		...
		PointQuadtree<MovingPoint> root = new PointQuadtree<>(points.get(0), 0, 0, width, height);

		// Now insert the rest of the points into the quadtree
		for (int i = 1; i < points.size(); i++) root.insert(points.get(i));
		...
		for (MovingPoint point : points){
			List<MovingPoint> collectp = root.findInCircle(point.getX(), point.getY(), 2*point.getR());
			if (collectp.size() > 1)
				colliders.add(point);
		}
