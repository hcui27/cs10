...
public class DotTreeGUI extends InteractiveGUI {
	...
	public void handleMousePress(int x, int y) {
		...
		if (mode == 'a') {
			...
			if (tree == null){
				tree = new PointQuadtree<>(point, 0, 0, width, height);}
	 ...
	 * hardcoded point locations for 800x600
	 */
	private void test1() { //passes test 1
		found = null;
	...
	//MY TEST CASES
	private void test2() { // because test1 wasn't working; tested individual lines of one to understand
		found = null;
		tree = new PointQuadtree<Point>(new Point(300, 400), 0, 0, 800, 600); // start with A
		tree.insert(new Point(150, 450)); // B
		tree.insert(new Point(250, 550)); // C
	...
	private void test3() { //because my test0 isnt working
		...
		tree.insert(new Point(250,550)); // C
		int bad = 0;
		bad += testFind(0,0,800,3,3,3);		// rect for all; circle for all; find all
		bad += testFind(390,299,10,3,2,0);	// rect for all; circle for A,B; find A
		bad += testFind(245,500,10,3,3,0);	// rect for all; circle for all; find C
		bad += testFind(137,438,200,3,3,2);	// rect for all; circle for all; find B, C
		bad += testFind(140,435,10,3,2,0);	// rect for all; circle for A,B; find none
		bad += testFind(750,545,10,2,1,0);	// rect for A,B; circle for A; find none
